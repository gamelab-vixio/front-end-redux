...
class Edit extends Component {
   ...
   componentWillMount() {
      ...
      document.title = "Vixio - Edit Story";
                                          ...
                                          paragraph.choices.map((choice, c_index) => {
                                             if(l_index === this.state.dataMapping.length - 1){
                                                if(!paragraph.links[c_index].hasOwnProperty('paragraph')){
                                                   return(
                                                      <div key={l_index+''+index+''+p_index+''+c_index} className="choices-wrapper animated fadeIn">
                                                         ...
                                                         <button className="btn add-link-button" onClick={() => this.openSideMenu(index, p_index, c_index)}>add link</button>
                                                      </div>
                                                   )
                                                }else{
                                                   return(
                                                      <div key={l_index+''+index+''+p_index+''+c_index} className="choices-wrapper animated fadeIn">
                                                         ...
                                                         <TextareaAutosize className="choices" rows={1} placeholder="Write your option here" value={choice} onChange={(e) => this.editChoice(index, p_index, c_index, e)} autoComplete="off" />
                                                         <button className="btn delete-option-button" onClick={() => this.deleteChoice(index, p_index, c_index)}><FaPlus size={15}/></button>
                                                         <button className="btn add-link-button" onClick={() => this.storyLinking(l_index, index, p_index, c_index)}>Go to link</button>
                                                      </div>
                                                   )
                                                }
                                             }else{
                        ...
                        section.paragraphs.map((paragraph, p_index) => {
                           return (
                              <div key={p_index} className="paragraph-wrapper">
                                 <div className="paragraph-sidebar" onClick={() => this.editLink(this.state.clickedIndex, this.state.clickedParagraphIndex, this.state.clickedChoiceIndex, index, p_index)}>
                                    <span>{"paragraph " + (p_index + 1) + ":"}</span><TextTruncate line={3} truncateText="…" text={paragraph.content}/>
                                 </div>
                              </div>   
               ...
               <div className="row no-gutters">
                  ...
                  <div className="col-12 col-sm-12 col-md-3 right-bar">
                     <div className="left-side-bar-wrapper">
                        ...
                        <input type="text" className="form-control" name="story_title" value={this.state.story_title} onChange={this.handleChange}/>

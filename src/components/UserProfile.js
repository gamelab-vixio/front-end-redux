...
class UserProfile extends Component {
   ...
   renderProfile() {
      ...
      return(
         <div className="user-profile-read">
            ...
            <button className="btn edit-profile" onClick={this.editProfile}>change image</button>
            {
               this.state.isEdit ? (
                  <div className="edit-profile-image">
                     ...
                     <button type="submit" name="photo" className="btn submit-edit-profile" onClick={this.uploadHandler}>upload image</button>
                  </div>
               ) : (
                  ''
               )
            ...
            <button type="submit" className="btn edit-profile" onClick={this.changePassword}>change password</button>
            {
               this.state.isChangePassword ? (
                  <div className="edit-profile-image">
                     ...
                     <input type="password" className="form-control" value={this.state.oldPassword} name="oldPassword" onChange={this.handleChangePasswordValue}/>
                     <label className="error-alert">{this.state.changePasswordErrorMessage.currentPassword}</label>

                     <label htmlFor="new-password">new password</label>
                     <input type="password" className="form-control" value={this.state.newPassword} name="newPassword" onChange={this.handleChangePasswordValue}/>
                     <label className="error-alert">{this.state.changePasswordErrorMessage.newPassword}</label>

                     <label htmlFor="confirm-new-password">confirm new password</label>
                     <input type="password" className="form-control" value={this.state.confirmPassword} name="confirmPassword" onChange={this.handleChangePasswordValue}/>
                     <label className="error-alert">{this.state.changePasswordErrorMessage.newPassword_Confirmation}</label>

                     <button className="btn submit-edit-profile" onClick={this.changePasswordHandler}>change password</button>
                  </div>
               ) : (
                  ''
               )
   ...
   uploadHandler(){ 
      ...
      UserService.userEditProfile(token, formData).then((res) => {
   ...
   changePasswordHandler() {
      ...
      UserService.userChangePassword(token, changePasswordData).then((res) => {
